...
public function getSchedule(){
	...
	foreach ($doc->getElementsByTagName("Game") AS $game) {
	  $obj->Games[] = (object)array("Week" => $game->getAttribute("Week"), "GameDate" => $game->getAttribute("GameDate"), "HomeTeam" => $game->getAttribute("HomeTeam"), "AwayTeam" => $game->getAttribute("AwayTeam"), "GameTime" => $game->getAttribute("GameTime"));
	}
...
public function getPlayers(){
	...
	$obj = new stdClass;
	$doc = DOMDocument::loadXML($data);
	if ($doc->getElementsByTagName("Error")->length > 0){$this->errorMsg = $doc->getElementsByTagName("Error")->item(0)->nodeValue;}
	$obj->Players = array();
	foreach ($doc->getElementsByTagName("Player") AS $player) {
	  $obj->Players[] = (object)array("playerId" => $player->getAttribute("playerId"), "Name" => $player->getAttribute("Name"), "Position" => $player->getAttribute("Position"), "Team" => $player->getAttribute("Team"));
	}
...
public function getPlayerDetails($playerId){
	...
	foreach ($doc->getElementsByTagName("Article") AS $article) {
	  $obj->News[] = (object)array("Title" => $article->getElementsByTagName("Title")->item(0)->nodeValue, "Source" => $article->getElementsByTagName("Source")->item(0)->nodeValue, "Published" => $article->getElementsByTagName("Published")->item(0)->nodeValue);
	}
...
public function getDraftRankings($position, $limit, $sos){
	...
	$obj->Players = array();
	foreach ($doc->getElementsByTagName("Player") AS $player) {
	  $p = array("playerId" => $player->getAttribute("playerId"), "PlayerName" => $player->getAttribute("Name"), "Position" => $player->getAttribute("Position"), "Team" => $player->getAttribute("Team"), "OverallRank" => $player->getAttribute("OverallRank"), "PositionRank" => $player->getAttribute("PositionRank"), "ByeWeek" => $player->getAttribute("ByeWeek"));
	  if ($player->getElementsByTagName("Week")->length > 0)
		{
		$weeks = array();
		foreach ($player->getElementsByTagName("Week") AS $week){$weeks[] = (object)array("WeekNumber" => $week->getAttribute("Number"), "Opponent" => $week->getAttribute("Opponent"), "DefensiveRank" => $week->getAttribute("DefensiveRank"));}
		}
	...
	if ($doc->getElementsByTagName("Team")->length > 0) {
		...
		foreach ($doc->getElementsByTagName("Injuries")->item(0)->getElementsByTagName("Team") AS $team) {
			...
			foreach ($team->getElementsByTagName("Injury") AS $inj)	{
				$obj->Injuries[] = (object)array("Week" => $inj->getElementsByTagName("Week")->item(0)->nodeValue, "playerId" => $inj->getElementsByTagName("playerId")->item(0)->nodeValue, "Player" => $inj->getElementsByTagName("PlayerName")->item(0)->nodeValue, "Team" => $inj->getElementsByTagName("Team")->item(0)->nodeValue, "Position" => $inj->getElementsByTagName("Position")->item(0)->nodeValue, "Injury" => $inj->getElementsByTagName("InjuryDesc")->item(0)->nodeValue, "PracticeStatus" => $inj->getElementsByTagName("PracticeStatusDesc")->item(0)->nodeValue, "GameStatus" => $inj->getElementsByTagName("GameStatusDesc")->item(0)->nodeValue, "Updated" => $inj->getElementsByTagName("LastUpdate")->item(0)->nodeValue);
			}
		}
	}
...
public function getWeeklyRankings($position, $week){
	...
	$obj->Players = array();
	foreach ($doc->getElementsByTagName("Player") AS $player) {
	  $p = array("playerId" => $player->getAttribute("playerId"), "PlayerName" => $player->getAttribute("Name"), "Position" => $player->getAttribute("Position"), "Team" => $player->getAttribute("Team"), "Week" => $player->getAttribute("Week"), "Rank" => $player->getAttribute("Rank"), "ProjectedPoints" => $player->getAttribute("ProjectedPoints"), "StandardPoints" => $player->getElementsByTagName("Standard")->item(0)->nodeValue, "StandardLow" => $player->getElementsByTagName("StandardLow")->item(0)->nodeValue, "StandardHigh" => $player->getElementsByTagName("StandardHigh")->item(0)->nodeValue, "PPR" => $player->getElementsByTagName("PPR")->item(0)->nodeValue, "PPRLow" => $player->getElementsByTagName("PPRLow")->item(0)->nodeValue, "PPRHigh" => $player->getElementsByTagName("PPRHigh")->item(0)->nodeValue);
	  $obj->Players[] = (object)$p;
	}
